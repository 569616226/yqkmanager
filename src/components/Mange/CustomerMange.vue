<template>
	<div>
		...
		<div class="know-container">
			<data-tables v-loading="loading" :data="CompaniesData" :table-props="tableProps" align="left" :show-action-bar="false" :custom-filters="customFilters" :pagination-def="paginationDef">
				<el-row slot="custom-tool-bar" style="margin-bottom: 10px" class="class_el_row">
					<el-col :span="24" class="text-align-left">
						<el-button type="primary" @click="goSeed">新增客户</el-button>
						<el-input v-model="customFilters[0].vals" prefix-icon="el-icon-search" placeholder="搜索客户名称" class="class_input_width">
						</el-input>
					</el-col>
				</el-row>
				<el-table-column prop="name" label="客户名称" key="name" sortable="custom">
		...
		<el-dialog title="企业信息详情" :visible.sync="previewcol" class="previewcol">
			<div class="cl-box" style="width:100%;">
				...
				<div class="cl-row">
					...
					<div class="cl-td">
						<img :src="colshowlog.logo" width="60px" />
					</div>
				</div>
				<div class="cl-row">
					<div class="cl-td">
		...
		methods: {
			...
			deletes(row) {
				this.$confirm('真的要删除此角色吗？', '删除', {
					...
					this.loading = true
					const data = await api.deleteCompanies(row.id)
					if(data.status) {
						this.$message({
							message: data.msg,
							type: 'success'
